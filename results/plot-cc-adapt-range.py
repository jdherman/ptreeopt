...
path = '../data/'
annQs = pd.read_csv(path + 'folsom-cc-annQ-MA30.csv', index_col=0, parse_dates=True)
anndmax = pd.read_csv(path + 'folsom-cc-7dmax.csv', index_col=0, parse_dates=True)
...
plt.annotate('RCP4.5', xy=(1550,3750), family='OfficinaSanITCMedium', color=rcpcolors[0])
plt.annotate('RCP6.0', xy=(1550,3550), family='OfficinaSanITCMedium', color=rcpcolors[1])
plt.annotate('RCP8.5', xy=(1550,3350), family='OfficinaSanITCMedium', color=rcpcolors[2])
...
for s in names:
  x = annQs['2099'][s].values[0]
  y = anndmax[s].max(axis=0)
  plt.scatter(x,y, 100, marker='s', linewidth=1.5, color='k', facecolor='None')
