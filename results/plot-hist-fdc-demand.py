...
df['flood_rule'] = pd.Series([flood_rule(d)
                              for d in df.water_day], index=df.index)
df['frac'] = (df.storage + df.inflow) / df.flood_rule
df['median_outflow'] = pd.Series([df.outflow[(df.water_day == d) & (
    df.frac < 0.8) & (df.outflow < 11)].median() for d in df.water_day], index=df.index)
df['mean_outflow'] = pd.Series([df.outflow[(df.water_day == d) & (
    df.frac < 0.8) & (df.outflow < 11)].mean() for d in df.water_day], index=df.index)
df['std_outflow'] = pd.Series([df.outflow[(df.water_day == d) & (
    df.frac < 0.8) & (df.outflow < 11)].std() for d in df.water_day], index=df.index)
...
plt.fill_between(range(366), mu + sig, mu - sig,
                 facecolor='steelblue', alpha=0.5, edgecolor='None')
...
plt.legend(['Historical $\mu \pm \sigma$',
            'MA$_{25}$ (centered)'], loc='upper left')
