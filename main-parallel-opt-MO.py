...
model = Folsom('folsom/data/folsom-daily.csv', sd='1995-10-01', ed='2015-09-30', 
  use_tocs = False, multiobj = True)
...
algorithm = PTreeOpt(model.f, 
                    ...
                    cx_prob = 0.70,
                    population_size = 100,
                    max_depth = 5, multiobj = True, epsilons=[0.01,1000.0,0.1,10.0]
                    )


snapshots = algorithm.run(max_nfe = 50000, log_frequency = 100)

pickle.dump(snapshots, open('output/snapshots-opt-depth-5-seed-' + str(comm.rank) + '.pkl', 'wb'))
